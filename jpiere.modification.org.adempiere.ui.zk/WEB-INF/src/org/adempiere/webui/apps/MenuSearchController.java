...
public class MenuSearchController implements EventListener<Event>{
	...
	private class MenuItemRenderer implements ListitemRenderer<MenuItem>, ListitemRendererExt {
		...
		public void render(Listitem item, MenuItem data, int index)
			...
			if (!Util.isEmpty(highlightText, true) && deleteAccents(data.getLabel()).toLowerCase().contains(deleteAccents(highlightText).toLowerCase())) { //JPIERE-0652
				// Space to maintain proper gap between icon and label
				cell.setLabel(" ");
				String label = data.getLabel();
				String unaccentedLabel = deleteAccents(label);//JPIERE-0652
				String matchString = deleteAccents(highlightText.toLowerCase());//JPIERE-0652
		 ...
		 */
		private static String deleteAccents(String text) {
		    String nfdNormalizedString = Normalizer.normalize(text, Normalizer.Form.NFKC); 
