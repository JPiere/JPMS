	...
	private ArrayList<Integer> getBelowColumns(Integer col)
	{
		...
		if (belowColumnMultiMap == null)
		{
			...
			for (Entry<Integer, Integer> entry : m_additionalLines.entrySet())
			{
				...
				// guard against null keys/values
				if (key == null || value == null)
					continue;
				belowColumnMultiMap.put(value, key);
			}
		}
...
//		int tempCurY = curY;	//JPIERE-0004-3 Modify TableElement#printColumn() by Hideaki Hagiwara
	...
	private Integer[] getPrintHeaderCol( int col ){
		...
		for (int dataCol = 0; dataCol < cols; dataCol++)
		{
			...
			if (m_additionalLines.containsKey(Integer.valueOf(dataCol)))
			{
				int col_new = ((Integer)m_additionalLines.get(Integer.valueOf(dataCol))).intValue();
				if( col_new == col ){
					list.add(Integer.valueOf(dataCol) );
				}
