...
{	
				...
				&& getPO(trx) instanceof DocAction)
			{
				...
				if (isInvoker())
				{
					...
					if(resp != null && resp.getResponsibleType().equals(MWFResponsible.RESPONSIBLETYPE_Organization))
					{
						...
						if(resp.getAD_Org_ID()==0)
						{
							orgInfo = MOrgInfo.get(getAD_Org_ID());//Get from WF Activity
						}else {
							orgInfo = MOrgInfo.get(resp.getAD_Org_ID()); //Get From WFResponsible
						}

						if(orgInfo.getSupervisor_ID() != 0)
						{
							setAD_User_ID(orgInfo.getSupervisor_ID());
						}else {
							m_docStatus = DocAction.STATUS_Invalid;
	...
	private boolean setVariable(String value, int displayType, String textMsg, Trx trx) throws Exception
	{
		...
		//JPIERE-0486 - start
		if("IsApproved=N".equals(msg))
		{
			msg = "[" + MRefList.getListName(getCtx(), 131, "NA") + "]";//131 = _Document Action
		}else if("IsApproved=Y".equals(msg)) {
			msg = "[" + MRefList.getListName(getCtx(), 131, "AP") + "]";
		}
		//JPIERE-0486 - End
	...
	public String getSummary()
	{
		...
		{
			int columnIndex = po.get_ColumnIndex("IsSOTrx");
			if(columnIndex == -1)
			{
				columnIndex = po.get_ColumnIndex("IsReceipt");

				if(columnIndex == -1)
				{
					sb.append(Msg.getElement(getCtx(), keyColumns[0])).append(" ");
				}else {
					sb.append(Msg.getElement(getCtx(), keyColumns[0],po.get_ValueAsBoolean("IsReceipt"))).append(" ");
				}

			}else {

				sb.append(Msg.getElement(getCtx(), keyColumns[0],po.get_ValueAsBoolean("IsSOTrx"))).append(" ");
			}
		}
		//JPIERE-0486 - End
