	...
	implements DocAction, ProcessCall, PaymentInterface, IDocsPostProcess
{
	...
	public boolean allocateIt()
	{
		...
		for (int i = 0; i < pAllocs.length; i++)
		{
			...
			//JPIERE-0214 - Start
			BigDecimal InvoiceAmt = pa.getInvoiceAmt();
...
//				allocationAmt = allocationAmt.add(pa.getOverUnderAmt());	//	overpayment (negative)
			...
			if (isReceipt())
			{
				...
				{
					aLine = new MAllocationLine (alloc, allocationAmt.compareTo(InvoiceAmt) > 0 ? InvoiceAmt : allocationAmt,
						pa.getDiscountAmt(), pa.getWriteOffAmt(), pa.getOverUnderAmt());
				}else {
					aLine = new MAllocationLine (alloc, allocationAmt.compareTo(InvoiceAmt) > 0 ? allocationAmt : InvoiceAmt,
							pa.getDiscountAmt(), pa.getWriteOffAmt(), pa.getOverUnderAmt());
				}
				...
				if(allocationAmt.signum() >= 0)
				{
					aLine = new MAllocationLine (alloc, allocationAmt.compareTo(InvoiceAmt) > 0 ? InvoiceAmt.negate() : allocationAmt.negate(),
						pa.getDiscountAmt().negate(), pa.getWriteOffAmt().negate(), pa.getOverUnderAmt().negate());

				}else {
					aLine = new MAllocationLine (alloc, allocationAmt.compareTo(InvoiceAmt) > 0 ? allocationAmt.negate() : InvoiceAmt.negate(),
							pa.getDiscountAmt().negate(), pa.getWriteOffAmt().negate(), pa.getOverUnderAmt().negate());
				}
			}
			//JPIERE-0214 - Finish
